...
export default function PrestigePills({ entity, entityName, onUpdate, threadId, entityOwnerId }) {
  ...
  const loadMyAwards = async (email) => {
    ...
    const logs = await base44.entities.AwardLog.filter({
      entity_id: entity.id,
      from_user: email,
    });
  ...
  const loadRecentAwards = async () => {
    const logs = await base44.entities.AwardLog.filter({ entity_id: entity.id }, "-created_date", 5);
  ...
  const award = async (field) => {
    ...
    await base44.entities[entityName].update(entity.id, updated);
    await base44.entities.AwardLog.create({
      entity_id: entity.id,
